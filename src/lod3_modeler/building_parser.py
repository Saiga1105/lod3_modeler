...
def parse_cityjson(cityjson_input, lod: str = "2.2"):
    ...
    for source_path, cityjson in cityjson_by_path.items():
        ...
        for object_id, city_object in cityjson.get("CityObjects", {}).items():
            ...
            if not lod_geometries:
                continue

            object_vertex_indices = sorted(set(
                vertex_index
                for geometry in lod_geometries
    ...
    if return_single:
        if not buildings:
            raise ValueError(f"No LoD {lod} building geometries found")
        return buildings[0]
    return buildings
